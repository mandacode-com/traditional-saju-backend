...
export class SajuController {
  ...
  @HttpCode(200)
  async getDailySaju(
    @Body(new ZodValidationPipe(DailySajuRequestSchema.omit({ userId: true, userName: true }))) body: Omit<DailySajuRequest, 'userId' | 'userName'>,
  ...
  @HttpCode(200)
  async getYearlySaju(
    @Body(new ZodValidationPipe(YearlySajuRequestSchema.omit({ userId: true, userName: true })))
